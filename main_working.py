...
from logger import LoggerManager, get_env_log_file_path, get_env_log_level
import argparse
from analyzer import ai_generate_query
from search import google_search
from search_count import check_and_confirm_api_count
...
def main(test_company_info=None):
    load_dotenv()
    config = load_config()
    log_file_path = get_env_log_file_path()
    log_level = get_env_log_level()
    logger = LoggerManager.get_logger("main", log_file_path, log_level)
    logger.info("main.py テスト起動: .envの読み込みとロガーの動作確認")
    logger.info(f"GOOGLE_API_KEY: {config['GOOGLE_API_KEY']}")
    logger.info(f"OLLAMA_API_URL: {config['OLLAMA_API_URL']}")
    ...
    for idx, query in enumerate(queries, 1):
        ...
        # API回数管理
        check_and_confirm_api_count(add_count=1, date_str=None)
